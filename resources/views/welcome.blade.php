                    ...
                    <div class="w-full max-w-sm">
                        @auth
                            <div class="text-center">
                                <h3 class="text-3xl font-bold text-text-primary mb-6">Welcome Back!</h3>
                                <p class="text-text-muted mb-8">You are already logged in.</p>
                        ...
                        @else
                            ...
                            <form class="space-y-6" method="POST" action="{{ route('login') }}">
                                ...
                                <div>
                                    ...
                                    <div class="mt-2">
                                        <input class="block w-full rounded-lg border-border bg-background-dark px-3 py-2 text-text-primary placeholder-text-muted focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary sm:text-sm transition-colors" id="email" name="email" placeholder="kamu@email.com" required="" type="email" value="{{ old('email') }}"/>
                                        @error('email')
                                ...
                                <div>
                                    ...
                                    <div class="mt-2">
                                        <input class="block w-full rounded-lg border-border bg-background-dark px-3 py-2 text-text-primary placeholder-text-muted focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary sm:text-sm transition-colors" id="password" name="password" required="" type="password"/>
                                        @error('password')
                                            <span class="text-error text-xs mt-1">{{ $message }}</span>
                                        @enderror
                                    </div>
