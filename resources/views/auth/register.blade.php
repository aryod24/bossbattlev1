...
<head>
    ...
    <title>CodeBossArena — Daftar</title>
    ...
    <style>
        ...
        @keyframes ornament-float-a { 0%,100%{transform:translate(0,0) rotate(-15deg);opacity:.55} 50%{transform:translate(8px,-18px) rotate(-12deg);opacity:1} }
        @keyframes ornament-float-b { 0%,100%{transform:translate(0,0);opacity:.6} 50%{transform:translate(-12px,14px);opacity:1} }
        @keyframes ornament-float-c { 0%,100%{transform:translate(0,0);opacity:.5} 50%{transform:translate(14px,-10px);opacity:.95} }
        @keyframes ornament-float-d { 0%,100%{transform:translate(0,0) scale(1);opacity:.55} 50%{transform:translate(-10px,-16px) scale(1.05);opacity:1} }
        @keyframes ornament-float-e { 0%,100%{transform:translate(0,0);opacity:.5} 50%{transform:translate(18px,12px);opacity:.9} }
        @keyframes ornament-float-f { 0%,100%{transform:translate(0,0);opacity:.6} 50%{transform:translate(-16px,-8px);opacity:1} }
            ...
            <div class="flex min-h-screen flex-col md:flex-row">
                ...
                <div class="hidden md:flex w-full md:w-3/5 flex-col justify-center items-start p-8 sm:p-12 md:p-12 lg:p-16 xl:p-20 relative min-h-[50vh] md:min-h-screen">
                    <main class="w-full max-w-3xl lg:max-w-4xl relative z-10">
                        ...
                        <h2 class="font-body mt-5 lg:mt-6 text-base md:text-lg lg:text-xl text-soft leading-relaxed max-w-2xl">
                            Daftar akun mahasiswa untuk mulai pre-test penempatan, kuasai materi, dan tantang boss.
                        </h2>
                ...
                <div class="auth-panel w-full md:w-2/5 flex items-center justify-center p-6 lg:p-8 min-h-screen">
                    <div class="w-full max-w-sm py-8">
                        ...
                        <h3 class="font-headline text-3xl font-extrabold text-center mb-2" style="color: #e5e2e3;">Daftar</h3>
                        <p class="font-body text-center text-sm text-soft mb-6">Buat akun mahasiswa untuk masuk arena.</p>

                        @if($errors->any())
                            <div class="mb-4 rounded-lg p-3 flex items-start gap-2.5"
                                 style="background-color: rgba(255, 99, 99, 0.08); border: 1px solid rgba(255, 99, 99, 0.3);">
                                <span class="material-symbols-outlined flex-shrink-0 mt-0.5" style="font-size: 18px; color: #ffb4ab;">error</span>
                                <ul class="font-body text-xs leading-relaxed space-y-0.5" style="color: #ffb4ab;">
                        ...
                        <form class="space-y-4" method="POST" action="{{ route('register') }}">
                            ...
                            {{-- NIM --}}
                            <div>
                                <label class="font-mono-label text-xs uppercase tracking-wider text-soft" for="nim">NIM</label>
                                <input class="cyber-input mt-2 block w-full rounded-lg px-3 py-2.5 text-sm"
                                       id="nim" name="nim" type="text"
                                       value="{{ old('nim') }}"
                                       placeholder="244107020001"
                                       maxlength="20" required autofocus/>
                            </div>

                            {{-- Nama --}}
                            <div>
                                <label class="font-mono-label text-xs uppercase tracking-wider text-soft" for="nama">Nama Lengkap</label>
                                <input class="cyber-input mt-2 block w-full rounded-lg px-3 py-2.5 text-sm"
                            ...
                            {{-- Kelas --}}
                            <div>
                                <label class="font-mono-label text-xs uppercase tracking-wider text-soft" for="kelas">Kelas</label>
                                <select class="cyber-input mt-2 block w-full rounded-lg px-3 py-2.5 text-sm"
                                        id="kelas" name="kelas" required>
                                    <option value="" disabled {{ old('kelas') ? '' : 'selected' }}>— Pilih kelas —</option>
                                    @foreach($allowedKelas as $kls)
                            ...
                            <div>
                                ...
                                <div class="mt-2" style="position: relative;">
                                    ...
                                    <button type="button" onclick="togglePassword('password','eye-icon')"
                                            style="position: absolute; top: 50%; right: 0.75rem; transform: translateY(-50%); background: none; border: none; cursor: pointer; color: #849495; padding: 0; display: flex; align-items: center;"
                            ...
                            <div>
                                ...
                                <div class="mt-2" style="position: relative;">
                                    ...
                                    <button type="button" onclick="togglePassword('password_confirmation','eye-icon-2')"
                                            style="position: absolute; top: 50%; right: 0.75rem; transform: translateY(-50%); background: none; border: none; cursor: pointer; color: #849495; padding: 0; display: flex; align-items: center;"
                                            aria-label="Toggle password visibility">
                                        <span id="eye-icon-2" class="material-symbols-outlined" style="font-size: 20px;">visibility_off</span>
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="pt-2">
                                <button class="btn-cyber-primary font-headline flex w-full items-center justify-center rounded-lg h-11 px-4 text-base font-bold tracking-wide" type="submit">
                                    <span class="truncate">Daftar &amp; Masuk</span>
                                </button>
                            </div>
                        </form>

                        <p class="font-body mt-6 text-center text-sm text-soft">
                            Sudah punya akun?
                            <a href="{{ route('login') }}" class="font-headline font-medium text-cyan-glow hover:underline">Masuk</a>
                        </p>
                    </div>
                </div>
            </div>
