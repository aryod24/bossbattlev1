<x-dosen-layout>
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6 bg-card p-6 rounded-2xl shadow-sm border border-border">
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('dosen.questions.create') }}" class="flex items-center justify-center gap-2 overflow-hidden rounded-lg h-11 px-5 bg-primary text-white text-sm font-bold leading-normal tracking-wide shadow-sm hover:bg-accent-hover">
    ...
    <div class="flex gap-2 p-1 bg-card border border-border rounded-lg mb-4 overflow-x-auto">
        @foreach($bankConfig as $bankId => $bank)
            <a href="{{ route('dosen.questions.index', ['bank' => $bankId, 'search' => request('search'), 'level' => request('level')]) }}" 
    ...
    <div id="bulkUploadSection" class="mb-4" style="display: none;">
        <div class="bg-card border border-border rounded-lg p-5">
            ...
            <form action="{{ route('dosen.questions.bulk-upload') }}" method="POST" enctype="multipart/form-data" class="flex items-end gap-4">
                @csrf
                <div class="flex-1">
    ...
    <script>
        ...
        function debounceSearch(input) {
            ...
            searchTimeout = setTimeout(() => {
                ...
                let url = '{{ route("dosen.questions.index") }}?bank=' + currentBank + '&search=' + encodeURIComponent(searchValue);
                if (currentLevel) {
        ...
        function filterByLevel(select) {
            ...
            const currentBank = '{{ $currentBank }}';
            let url = '{{ route("dosen.questions.index") }}?bank=' + currentBank + '&level=' + encodeURIComponent(levelValue);
            if (currentSearch) {
